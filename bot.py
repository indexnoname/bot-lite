...
def convert_image_to_scheme(image, name):
    ...
    pack_0 = struct.pack(">b", 0)
    pack_5 = struct.pack(">b", 5)

    for y in range(height):
        for x in range(width):
            buffer.write(pack_0 + struct.pack(">HH", x, height - y - 1) + pack_5 + struct.pack(">Hb", config_map[tuple(new_pixels[y, x])], 0))
...
async def convert(ctx, scale: int = 100, resample_method: str = 'LANCZOS'):
    ...
    with Image.open(imageio).convert('RGB') as image:
        image = resize_image(image, scale, resample_method.upper())[0]  # Unpack only the resized image
