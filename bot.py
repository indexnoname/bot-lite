...
def resize_image(image, scale, resample_method='LANCZOS'):
    ...
    return image.resize((target_width, target_height), getattr(Image, resample_method, Image.LANCZOS)), target_width, target_height

def txtbin(txt = str):
    return struct.pack(">H", len(txt))+txt.encode("UTF-8")

def convert_image_to_scheme(image, name, height, width):
